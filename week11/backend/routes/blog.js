...
router.get("/blogs/:id", function (req, res, next) {
  ...
  const promise2 = pool.query(
    "select * from comments LEFT OUTER JOIN images on comments.id = images.comment_id where comments.blog_id = ? ORDER BY comment_date desc",
    [req.params.id]
  );
...
router.post(
  ...
  async function (req, res, next) {
    ...
    try {
      let results = await conn.query(
        "INSERT INTO `comments`(blog_id, comment, `like`) VALUES(?, ?, 0);",
        [blogId, comment]
      );      
      ...
      await conn.query(
        "INSERT INTO `images`(images.blog_id, images.comment_id, images.file_path) VALUES(?, ?, ?);",
